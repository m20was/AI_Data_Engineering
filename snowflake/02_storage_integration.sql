...
--   A. In AWS IAM, create role `snowflake-food-delivery-role` with a PLACEHOLDER
...
-- >>> EDIT THESE TWO <<<
--   <ROLE_ARN> = arn:aws:iam::<your-account-id>:role/snowflake-food-delivery-role
--   <BUCKET>   = your bucket, e.g. food-delivery-dl-yourname
CREATE OR REPLACE STORAGE INTEGRATION FOOD_DELIVERY_S3_INT
  TYPE = EXTERNAL_STAGE
  STORAGE_PROVIDER = 'S3'
  ENABLED = TRUE
  STORAGE_AWS_ROLE_ARN = 'arn:aws:iam::566167302601:role/snowflake-s3-role'
  STORAGE_ALLOWED_LOCATIONS = ('s3://m20was-food-delivery/');

GRANT USAGE ON INTEGRATION FOOD_DELIVERY_S3_INT TO ROLE DBT_ROLE;

-- Run this, then copy the two values into the IAM role trust policy (Step D).
DESC INTEGRATION FOOD_DELIVERY_S3_INT;
